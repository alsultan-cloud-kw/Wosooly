from products.db_helper import *
import pandas as pd
from sklearn.cluster import KMeans
from sklearn.preprocessing import StandardScaler
from datetime import datetime

def function_get_top_selling_products(db, client_id):
    top_selling_products_response = get_top_selling_products_data(db, client_id)
    return top_selling_products_response

def function_get_top_selling_products_inbetween(db, client_id, start_date, end_date):

    top_selling_products_inbetween_response = get_top_selling_products_inbetween_data(db, client_id, start_date, end_date)

    return top_selling_products_inbetween_response

def function_get_products_sales_table(db, client_id, start_date, end_date):

    products_sales_table_response = get_products_sales_table_data(db, client_id, start_date, end_date)
